...
namespace solution {
	...
	private:
		...
		struct node_type
		{
			...
			//4.2.3
			int64_t add_value; //4.2.3

			//4.2.4
			bool assigned;
			int64_t new_value; //4.2.4
		...
		void swap(node_type *&, node_type *&, uint32_t); //4.3.0

		uint32_t find_greater_pos(node_type *, int64_t);//4.3.1
		uint32_t find_lesser_pos(node_type *, int64_t);//4.3.1
	...
	public:
		...
		//4.2.3
		void add(int64_t, uint32_t, uint32_t); //4.2.3

		//4.2.4
		void assign(int64_t, uint32_t, uint32_t); //4.2.4

		//4.3.2
		void next_permutation(uint32_t, uint32_t);
		void prev_permutation(uint32_t, uint32_t); //4.3.2
	};
	...
	//4.2.3-----------------------------------------------------------------------
	void cartesian_tree::node_type::add(int64_t x)
	{
		value += x;
		left_value += x;
		right_value += x;
	...
	void cartesian_tree::node_type::add_push()
	{
		if (add_value)
		{
			if (left_child != nullptr)
				left_child->add(add_value);
			if (right_child != nullptr)
				right_child->add(add_value);
			add_value = 0;
		}
	}

	void cartesian_tree::add(int64_t x, uint32_t l, uint32_t r)
	{
		node_type *L, *M, *R;
		split(root, L, M, R, l, r);
		M->add(x);
	...
	//4.2.4-----------------------------------------------------------------------
	void cartesian_tree::node_type::assign(int64_t x)
	{
		assigned = true;
		add_value = 0;
		left_value = right_value = value = new_value = x;
		sum = size * new_value;
		no_dec_pref = no_dec_suff = no_inc_pref = no_inc_suff = size;
	}

	void cartesian_tree::node_type::assign_push()
	{
		if (assigned)
		{
			assigned = false;
			if (left_child != nullptr)
				left_child->assign(new_value);
			if (right_child != nullptr)
				right_child->assign(new_value);
		}
	}

	void cartesian_tree::assign(int64_t x, uint32_t l, uint32_t r)
	{
		node_type *L, *R, *M;
		split(root, L, M, R, l, r);
		M->assign(x);
	...
	//4.3.0-----------------------------------------------------------------------
	void cartesian_tree::swap(node_type *&simple_node, node_type *&tree, uint32_t position)
	{
		node_type *L, *M, *R;
		split(tree, L, M, R, position, position);
		merge(L, simple_node, R, tree);
		simple_node = M;
	}
	//4.3.0-----------------------------------------------------------------------

	//4.3.1-----------------------------------------------------------------------
	uint32_t cartesian_tree::find_greater_pos(node_type *v, int64_t x)
	{
		v->push();
		if (v->right_child != nullptr && v->right_child->left_value > x)
			return size(v->left_child) + 1 + find_greater_pos(v->right_child, x);
		if (v->value > x)
			return size(v->left_child) + 1;
		return find_greater_pos(v->left_child, x);
	}

	uint32_t cartesian_tree::find_lesser_pos(node_type *v, int64_t x)
	{
		v->push();
		if (v->right_child != nullptr && v->right_child->left_value < x)
			return size(v->left_child) + 1 + find_lesser_pos(v->right_child, x);
		if (v->value < x)
			return size(v->left_child) + 1;
		return find_lesser_pos(v->left_child, x);
	}
	//4.3.1-----------------------------------------------------------------------

	//4.3.2-----------------------------------------------------------------------
	void cartesian_tree::next_permutation(uint32_t l, uint32_t r)
	{
		node_type *L, *M, *R;
		split(root, L, M, R, l, r);
		if (M->no_inc_suff == M->size)
		{
			M->reverse();
			merge(L, M, R, root);
			return;
		...
		node_type *X, *Y, *Z;
		int no_suff_len(M->size - M->no_inc_suff);
		split(M, X, Y, Z, no_suff_len, no_suff_len);
		swap(Y, Z, find_greater_pos(Z, Y->value));
		Z->reverse();
		merge(Y, Z, M);
		merge(X, M, M);
		merge(L, M, R, root);
	...
	void cartesian_tree::prev_permutation(uint32_t l, uint32_t r)
	{
		node_type *L, *M, *R;
		split(root, L, M, R, l, r);
		if (M->no_dec_suff == M->size)
		{
			M->reverse();
			merge(L, M, R, root);
			return;
		...
		node_type *X, *Y, *Z;
		int no_suff_len(M->size - M->no_dec_suff);
		split(M, X, Y, Z, no_suff_len, no_suff_len);
		swap(Y, Z, find_lesser_pos(Z, Y->value));
		Z->reverse();
		merge(Y, Z, M);
		merge(X, M, M);
		merge(L, M, R, root);
...
namespace debug {
	...
	struct {
		...
		{
			std::prev_permutation(a.begin() + l, a.begin() + r + 1);
		}
	} dbg;
}
...
void solve()
{
	...
	solution::cartesian_tree array(input);
#ifdef DEBUG_MODE
	...
	for (uint32_t counter(0); counter < q; ++counter)
	{
		...
		if (t == 6 || t == 7)
		{
			std::cin >> l >> r;
#ifdef DEBUG_MODE
			if (t == 6) dbg.next_permutation(l, r);
			else dbg.prev_permutation(l, r);
#endif
			++l, ++r;
			if (t == 6)
