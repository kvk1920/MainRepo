...
namespace cartesian_tree
{
	...
	struct node
	{
		...
		void recalc()
		{
			...
			if (left)
			{
				linc = left->linc + (left->sz == left->linc && left->rval <= val);
				ldec = left->ldec + (left->sz == left->ldec && left->rval >= val);
			}
			if (right)
			{
			...
			if (right)
			{
				rinc = right->rinc + (right->sz == right->rinc && right->lval >= val);
				rdec = right->rdec + (right->sz == right->rdec && right->lval <= val);
			}
			if (left)
			{
			...
			if (!left || (val >= left->rval && linc == left->sz)) ++linc;
			if (!left || (val <= left->rval && ldec == left->sz)) ++ldec;
			if (right)
			{
				if (linc == sz - right->sz && val <= right->lval) linc += right->linc;
				if (ldec == sz - right->sz && val >= right->lval) ldec += right->ldec;
			}
			rinc = rdec = 0;
			if (right)
			{
			...
			if (!right || (val <= right->lval && rinc == right->sz)) ++rinc;
			if (!right || (val >= right->lval && rdec == right->sz)) ++rdec;
			if (left)
			{
				if (rinc == sz - left->sz && val >= left->rval) rinc += left->rinc;
				if (rdec == sz - left->sz && val <= left->rval) rdec += left->rdec;
			}
		}
...
int main()
{
	...
	while (q--)
	{
		...
		t = 1 + rand() % 7;
		if (t == 2) x = rand() % 10000, pos = rand() % sz;
			//cin >> x >> pos;
		else if (t == 3) pos = rand() % sz;
			//cin >> pos;
		else if (t == 4 || t == 5) l = rand() % sz, r = rand() % sz, x == rand() % 10000;
			//cin >> x >> l >> r;
